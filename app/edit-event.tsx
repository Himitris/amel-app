...
export default function EditEventScreen() {
    ...
    useEffect(() => {
        const loadEvent = async () => {
            ...
            setEventType(event.eventType || 'professional');
            setSelectedDate(startDate);
            setStartTime(startDate);
            setDuration(calculatedDuration);
            setSelectedColor(event.color || COLORS_PALETTE[4]);
            setClientAddress(event.location || '');
            setMessage(event.description || '');
            
            if (event.eventType === 'professional') {
                // Extraire les informations du titre (format: "Service - Client")
                const titleParts = event.title.split(' - ');
                if (titleParts.length >= 2) {
                    setService(event.service || titleParts[0]);
                    setClientName(titleParts[1]);
                } else {
    ...
    const handleUpdateEvent = async () => {
        try {
            ...
            // Vérifier si l'heure ou la date a changé pour mettre à jour les créneaux
            const originalStartDate = new Date(originalEvent.startDate);
            const dateChanged = 
                originalStartDate.getDate() !== startDate.getDate() ||
                originalStartDate.getMonth() !== startDate.getMonth() ||
                originalStartDate.getFullYear() !== startDate.getFullYear() ||
                originalStartDate.getHours() !== startDate.getHours() ||
                originalStartDate.getMinutes() !== startDate.getMinutes();

            await updateEvent(originalEvent.id, eventData, dateChanged);
